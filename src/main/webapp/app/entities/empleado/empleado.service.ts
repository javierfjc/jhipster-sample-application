...
export class EmpleadoService {
  ...
  protected convertDateFromClient(empleado: IEmpleado): IEmpleado {
    const copy: IEmpleado = Object.assign({}, empleado, {
      antiguedad: empleado.antiguedad != null && empleado.antiguedad.isValid() ? empleado.antiguedad.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.antiguedad = res.body.antiguedad != null ? moment(res.body.antiguedad) : null;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((empleado: IEmpleado) => {
        empleado.antiguedad = empleado.antiguedad != null ? moment(empleado.antiguedad) : null;
      });
    }
    return res;
  }
